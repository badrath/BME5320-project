...
def filter (queryMatch, MGG):
    ...
    match_data = {
        "query"       :   str(query_data[0]).split('.')[0],
        "target"      :   str(query_data[1]).split('.')[0],
        "perc_id"     :   float(query_data[2]),
        "align_len"   :   float(query_data[3]),
        "mismatches"  :   float(query_data[4]),
        "gap_opens"   :   float(query_data[5]),
        "q_start"     :   float(query_data[6]),
        "q_end"       :   float(query_data[7]),
        "t_start"     :   float(query_data[8]),
        "t_end"       :   float(query_data[9]),
        "e_val"       :   float(query_data[10]),
        "bit_score"   :   float(query_data[11])
        };
    
    target = None;
    if(match_data['target'] not in MGG.keys()): #filters out MGG self hits, either same as the query search or other MGG genes/variations
        
#         sys.stdout.write('[DEV]    query from match_data: ' + match_data['query'] + '\n');
#         sys.stdout.write('[DEV]    MGG keys: ' + str(keys(MGG)) + '\n');
...
#     sys.stdout.write('[DEV]    returned target: ' + str(target) + '\n');
    return(target);
