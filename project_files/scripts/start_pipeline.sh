...
echo "[INFO]	BLASTP array job id: $hold_blast_jid"


##########################################
#for blast results, WORKS!
...
#hold_blastdbcmd_jid=$(qsub -pe smp 1 -terse -hold_jid $hold_blast_jid -t 1-2:1 submit_name_find_array.job | awk -F. '{print $1}') #	This will filter then run blastdbcmd and save results in seprate files according to the .faa.out files, runs for test of whole start_pipeline.sh
#hold_blastdbcmd_jid=$(qsub -pe smp 1 -terse -t 1-1:1 submit_name_find_array.job | awk -F. '{print $1}') #	This will filter then run blastdbcmd and save results in seprate files according to the .faa.out files, for section testing only
	#	above currently set for n=1 array elements for debugging, on dev test run with atleast n=2
echo "[INFO]	filter and find functional names array job id: $hold_blastdbcmd_jid, waits on blast array before beginning."
...
hold_annotation_jid=$(qsub -pe smp 1 -terse -hold_jid $hold_blastdbcmd_jid amalgamateFxNames.job) ##	This will combine the results of the individual sequence runs into 2 files, an Annotated_seq_simple.csv and Annotated_seqes_long.csv. not written
#qsub -pe smp 1 -terse amalgamateFxNames.job #	for testing and dev only, does not wait for completion of anything before it
echo "[INFO]	amalgamate annotation results job id: $hold_annotation_jid, waits on filtering and fx naming before beginning."
